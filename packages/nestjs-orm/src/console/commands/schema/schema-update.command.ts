  ...
  async run(inputs: string[], options?: SchemaUpdateOptions): Promise<void> {
    try {
      ...
      if (!options?.safe && !options?.dryRun) {
        this.logger.warn('⚠️  WARNING: schema:update can cause data loss!');
        this.logger.warn('⚠️  Use --safe mode or --dry-run first to review changes');
        this.logger.warn('⚠️  For production, use migrations instead!\n');
      }
      ...
      if (options?.safe) {
        this.logger.log('\n📌 Safe mode was used - no tables or columns were dropped');
      }
    } catch (error: any) {
      this.logger.error('❌ Failed to update schema:', error.message);
      throw error;
    }
  }
