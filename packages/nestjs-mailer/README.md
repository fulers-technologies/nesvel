...
Comprehensive email sending module for NestJS using [@nestjs-modules/mailer](https://github.com/nest-modules/mailer) with React template support.
...
await mailerService.sendMailToMany(recipients, {
  subject: 'Newsletter',
  template: 'newsletter',
  context: {
    month: 'January',
...
await mailerService.sendTransactionalEmail({
  to: user.email,
...
await mailerService.sendMarketingEmail({
  to: user.email,
