...
export function Route<TBody = any>(options: RouteOptions<TBody>): MethodDecorator;
...
export function Route<TBody = any>(
  ...
  } else if (typeof pathOrOptions === 'string' || Array.isArray(pathOrOptions)) {
  ...
  if (!exclude) {
    ...
    const docs = autoGenerateDocs(methodStr, pathStr, documentation ?? undefined);
  ...
  if (finalOptions.deprecated) {
    ...
    if (finalOptions.deprecated.sunset) {
      decorators.push(Header('Sunset', finalOptions.deprecated.sunset.toUTCString()));
    }

    // Add deprecation notice to response header
    decorators.push(
      Header('X-API-Deprecated', finalOptions.deprecated.reason || 'This endpoint is deprecated')
    );
  ...
  if (finalOptions.throttle) {
    if (finalOptions.throttle.limit !== undefined || finalOptions.throttle.ttl !== undefined) {
      decorators.push(
  ...
  if (finalOptions.security) {
    ...
    if (finalOptions.security.rateLimit) {
      decorators.push(SetMetadata('rateLimit', finalOptions.security.rateLimit));
    }
    if (finalOptions.security.ipWhitelist) {
      decorators.push(SetMetadata('ipWhitelist', finalOptions.security.ipWhitelist));
    }
