...
import { MailTransportType, IMailerConfig } from '@nesvel/nestjs-mailer';
 ...
 * const transport = mailerConfig.transport;
 * const from = mailerConfig.from;
 * const smtpHost = mailerConfig.smtp?.host;
 * ```
 */
export const mailerConfig: IMailerConfig = {
   ...
   */
  transport: (process.env.MAIL_TRANSPORT as MailTransportType) || MailTransportType.SMTP,
   ...
   *
   * Path to email template files (relative or absolute)
   *
